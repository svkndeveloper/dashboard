    ...
    builder
      ...
      .addCase(completeCardThunk.fulfilled, (state, action) => {
        ...
        state.cards = state.cards.filter(
          item => item._id !== action.payload.completedCard._id
        );
      ...
      .addCase(getCardsThunk.fulfilled, (state, action) => {
        state.isLoading = false;
        state.error = null;
        state.cards = action.payload.cards.filter(
          item => item.status === 'Incomplete'
        );
        state.doneCards = action.payload.cards.filter(
      ...
      .addCase(deleteCardThunk.fulfilled, (state, action) => {
        ...
        state.error = null;
        state.cards = state.cards.filter(
          item => item.status === 'Incomplete' && item._id !== action.payload
        );
        state.doneCards = state.doneCards.filter(
          item => item.status === 'Complete' && item._id !== action.payload
        );
      })
